...
)
{
	...
	auto const labelsWithNewGhosts = nameDispenser.generateNewLabels(_block, _dialect);
		...
		for (Scope::Function const* function: m_dfg.functions)
		{
			...
			functionLabels[&functionInfo] = useNamedLabel ?
				m_assembly.namedLabel(
					std::string{_labels[function->name]},
	...
	::createStackLayout(
		...
		[&](unsigned _i)
		{
			...
			else
			{
				...
				std::string msg =
					"Cannot swap " + (ASTLabelRegistry::empty(varNameDeep) ? "Slot " + stackSlotToString(deepSlot, m_labels, m_dialect) : "Variable " + std::string{m_labels[varNameDeep]}) +
					" with " + (ASTLabelRegistry::empty(varNameTop) ? "Slot " + stackSlotToString(m_stack.back(), m_labels, m_dialect) : "Variable " + std::string{m_labels[varNameTop]}) +
					": too deep in the stack by " + std::to_string(deficit) + " slots in " + stackToString(m_stack, m_labels, m_dialect);
		...
		[&](StackSlot const& _slot)
		{
			...
			if (auto depth = util::findOffset(m_stack | ranges::views::reverse, _slot))
			{
				...
				else if (!canBeFreelyGenerated(_slot))
				{
					...
					std::string msg =
						(ASTLabelRegistry::empty(varName) ? "Slot " + stackSlotToString(_slot, m_labels, m_dialect) : "Variable " + std::string{m_labels[varName]})
						+ " is " + std::to_string(*depth - 15) + " too deep in the stack " + stackToString(m_stack, m_labels, m_dialect);
					m_stackErrors.emplace_back(StackTooDeepError(
						m_currentFunctionInfo ? m_currentFunctionInfo->function.name : YulName{},
						varName,
						deficit,
