...
)
{
	...
	std::set<std::string> externallyUsedIdentifiers = _externallyUsedFunctions + _localVariables;
	...
	) -> bool
	{
		if (_insideFunction)
			return false;
		return util::contains(_localVariables, parserResult->labels()[_identifier.name]);
	};
	...
	)
	{
		...
		auto it = std::find(_localVariables.begin(), _localVariables.end(), parserResult->labels()[_identifier.name]);
